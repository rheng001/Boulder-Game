...
uint8_t getPlayer1 = 0x00;

unsigned char gameOver = 0;
unsigned char reset = 0;

unsigned char player1Lose = 0x00;
unsigned char player1Received = 0x00;
...
void displayScreen()
{
	...
	if(gameOver != 1)
	{
		...
		//Current Score
		LCD_Cursor(23);
		LCD_WriteData( tmpScore / 100  + '0'); //hundres place
		...
		else if(gameOver == 1 && player1Lose != 1)
		{
		sendGameOver();
		LCD_DisplayString(1, "You Lose        Player 1 Wins"); //I lose
		_delay_ms(1);
		}
		else if (gameOver == 1 && player1Lose == 1) //I win
		{
		sendGameOver();
		LCD_DisplayString(1, "You Win         Player 1 Loses");
		_delay_ms(1);
		}
		}
		...
		case moveLoser:
		if(gameOver != 1 && player1Received == 1)
		{
		USART_Send(0b01100110, 1); //Send P2 Not Lose
		player1Lose = 1;
		_delay_ms(100);
		}
		else
		...
		case receiveLoser:
		getPlayer1 = USART_Receive(1);
		if(getPlayer1 > 0) //Player 1 Loses
		{
			player1Received = 1;
		}
		else
